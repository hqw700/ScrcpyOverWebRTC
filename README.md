...
- **极致流畅**: 采用零扫描流解析 (Zero-Search Parsing)，不引入新的内存拷贝，性能和原生 scrcpy 基本一致。
...
- **安卓 App**: 官方 App 主控/被控一体：作为主控端可直接操控手机（多点触控、音频、双向剪切板）；作为被控端支持 Root 与 Shizuku (ADB) 双引擎免电脑运行 Agent；开启单机模式后服务端亦可运行在手机内，彻底摆脱服务器。
- **全设备适配**: 完美兼容 Android 物理真机 (Root / 免 Root)、Android 模拟器、redroid 虚拟化容器以及各类第三方商业云手机环境。
- **设备分享**: （v0.3.2）支持在内网或者公网通过免密链接或者卡密分享机器，支持设置时长和控制方式。本地部署且有IPv6的，可以直接通过IPv6地址公网访问。
